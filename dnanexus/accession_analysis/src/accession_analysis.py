...
def get_encoded_repn(mapping_analysis):
	#this is a fragile way to infer the rep number.  It depends on the name of the mapping analysis.  But since there is 
	#nowhere in the anlysis input to put it, the only alternative is to infer it from the list of fastq accessions in
	#the inputs.
	#Probably the best thing is to add that as an input into the analysis, then it would be recorded in the analysis metadata
	logger.debug("in get_encoded_repn with analysis[name] %s" %(mapping_analysis['name']))
	m_name = re.search('Map ENCSR[0-9]{3}[A-Z]{3} rep(\d+)', mapping_analysis['name'])
	if not m_name:
		logger.error("Could not infer ENCODED repn from analysis name: %s" %(mapping_analysis['name']))
		return
	else:
		logger.debug("in get_encoded_repn and found repn to be %s" %(m_name.group(1)))
...
def get_control_mapping_stages(peaks_analysis, experiment, keypair, server, reps=[1,2]):
	...
	#here reps is always 1,2 because the peaks job rep numbers are 1,2 ... these are not the ENCODEd biological_replicate_numbers, which are
	#only known to the analysis via its name, or by going back to ENCODEd and figuring out where the fastqs came from
...
def get_histone_peak_stages(peaks_analysis, mapping_stages, control_stages, experiment, keypair, server):

	logger.debug('in get_histone_peak_stages with peaks_analysis %s; experiment %s' %(peaks_analysis['id'], experiment['accession']))
...
def get_tf_peak_stages(peaks_analysis, mapping_stages, control_stages, experiment, keypair, server):

	logger.debug('in get_tf_peak_stages with peaks_analysis %s; experiment %s' %(peaks_analysis['id'], experiment['accession']))
	logger.debug('and len(mapping_stages) %d len(control_stages) %d mapping_stages' %(len(mapping_stages), len(control_stages)))
	...
	peak_stages = { #derived_from is by name here, will be patched into the file metadata after all files are accessioned
		...
		"SPP Peaks" : {
			'output_files': [
				{'name': 'rep1_peaks',		'derived_from': [rep1_bam] + rep1_ctl,	'metadata': narrowpeak_metadata},
				{'name': 'rep2_peaks',		'derived_from': [rep2_bam] + rep2_ctl,	'metadata': narrowpeak_metadata},
				{'name': 'pooled_peaks',		'derived_from': [rep1_bam, rep2_bam] + pooled_ctl_bams,			'metadata': narrowpeak_metadata},
				{'name': 'rep1_peaks_bb',		'derived_from': ['rep1_peaks'],		'metadata': narrowpeak_bb_metadata},
				{'name': 'rep2_peaks_bb',		'derived_from': ['rep2_peaks'],		'metadata': narrowpeak_bb_metadata},
				{'name': 'pooled_peaks_bb',	'derived_from': ['pooled_peaks'],		'metadata': narrowpeak_bb_metadata}
			],
			'qc': [],
			'stage_metadata': {} # initialized below
		},
		...
		"Final IDR peak calls": {
			'output_files': [
				{'name': 'conservative_set',		'derived_from': ['rep1_peaks', 'rep2_peaks', 'pooled_peaks'], 'metadata': idr_conservative_narrowpeak_metadata},
				{'name': 'conservative_set_bb',	'derived_from': ['conservative_set'], 'metadata': idr_conservative_narrowpeak_bb_metadata},
				{'name': 'optimal_set',		'derived_from': ['rep1_peaks', 'rep2_peaks', 'pooled_peaks'], 'metadata': idr_optimal_narrowpeak_metadata},
				{'name': 'optimal_set_bb',	'derived_from': ['optimal_set'], 'metadata': idr_optimal_narrowpeak_bb_metadata}
			],
			'qc': ['reproducibility_test', 'rescue_ratio', 'Np', 'N1', 'N2', 'Nt', 'self_consistency_ratio'],
			'stage_metadata': {} # initialized below
		}
	}
